...
$('#calculate').on('click', function () {
    ...
    var registerClassBeforeUpgrade = +$('#register-class-before-upgrade').val();
    var registerClassAfterUpgrade = +$('#register-class-after-upgrade').val();
    ...
    for (var i = 3; i < (rowCellsNumber - 1); i++) {
        ...
        if(registerClassAfterUpgrade && registerClassBeforeUpgrade) {
            result = +$(tableRows[6]).find('td:eq(' + i + ')').html() +
                ( (18.3 + 0.31 * registerClassAfterUpgrade) - (18.3 + 0.31 * registerClassBeforeUpgrade)  );
